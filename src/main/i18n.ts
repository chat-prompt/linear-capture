...
export function t(key: string, options?: Record<string, any>): string {
   ...
   if (key === 'capture.title' || key === 'settings.title') {
     logger.log(`[i18n] t('${key}') lang=${i18next.language} => '${result}'`);
   }
   return result;
 }

 export async function changeLanguage(lng: string): Promise<void> {
   logger.log(`[i18n] changeLanguage called: ${lng}, current: ${i18next.language}`);
   await i18next.changeLanguage(lng);
   logger.log(`[i18n] changeLanguage done: now ${i18next.language}`);
 }
