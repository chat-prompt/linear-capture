...
│       └── auto-updater.ts    # 자동 업데이트 (electron-updater)
...
| `check-for-updates` | renderer→main | 수동 업데이트 확인 요청 |
| `get-app-version` | renderer→main | 현재 앱 버전 조회 |
...
npm run reinstall        # 완전 클린 재설치 (빌드 포함, 권한 리셋)
npm run reinstall:quick  # 빠른 재설치 (빌드 스킵, 권한 리셋)
```

## 🔧 권한 문제 해결 (완전 클린 재설치)

앱 삭제 후 재설치 시 화면 녹화 권한이나 핫키가 작동하지 않는 경우:

### 방법 1: npm 스크립트 사용 (권장)

```bash
# 전체 재설치 (DMG 빌드 + 권한 리셋 + 설치)
npm run reinstall

# 빠른 재설치 (기존 DMG로 권한 리셋 + 설치)
npm run reinstall:quick
```
...
# 접근성 권한 리셋 (핫키 관련)
tccutil reset Accessibility com.gpters.linear-capture
...
2. Linear Capture 앱 우클릭 → "열기" 선택 (Gatekeeper 우회)
3. `⌘+Shift+L`로 캡처 시도
4. 화면 녹화 권한 팝업이 뜨면 "허용" 클릭
5. 시스템 환경설정 → 보안 및 개인 정보 보호 → 화면 녹화에서 Linear Capture 체크 확인
...
GitHub Releases를 통한 자동 업데이트 시스템. 앱 시작 시 새 버전을 확인하고, 사용자 동의 후 다운로드 및 설치.
...
사용자가 "Download" 클릭
    ↓
ZIP 파일 다운로드 (delta 업데이트 지원)
    ↓
SHA512 해시 검증
    ↓
다운로드 완료 → "Restart Now" 다이얼로그
    ↓
앱 재시작 시 자동 설치
```
...
1. **자동 확인**: 앱 시작 5초 후, 이후 4시간마다 자동 확인
2. **수동 확인**: Settings → "업데이트 확인" 버튼 클릭

### 핵심 파일

| 파일 | 설명 |
|------|------|
| `src/services/auto-updater.ts` | electron-updater 설정 및 이벤트 핸들러 |
| `release/latest-mac.yml` | 최신 버전 메타데이터 (빌드 시 자동 생성) |
| `.github/workflows/release.yml` | GitHub Actions 자동 배포 워크플로우 |
| `dev-app-update.yml` | 개발 환경 테스트용 설정 |

### 빌드 결과물

`npm run dist:mac` 실행 시 생성되는 파일:

| 파일 | 용도 |
|------|------|
| `Linear Capture-x.x.x-universal.dmg` | 수동 설치용 |
| `Linear Capture-x.x.x-universal-mac.zip` | 자동 업데이트용 (필수) |
| `latest-mac.yml` | 버전 정보 + SHA512 해시 |
| `*.blockmap` | Delta 업데이트용 (부분 다운로드) |
...
#    → macOS에서 빌드
#    → DMG + ZIP + latest-mac.yml 생성
#    → GitHub Releases에 업로드
```

### Ad-hoc 서명 관련

- Apple Developer 인증서 없이 ad-hoc 서명 사용
- 코드 서명 검증 대신 **SHA512 해시 검증**으로 업데이트 무결성 보장
- 최초 설치 시 Finder에서 우클릭 → "열기" 필요 (Gatekeeper 우회)

### electron-updater 설정

```typescript
// auto-updater.ts
autoUpdater.autoDownload = false;        // 자동 다운로드 안 함
...
Settings 창 하단에 버전 정보와 "업데이트 확인" 버튼 추가:

```
┌─────────────────────────────────────┐
│ Settings                            │
│                                     │
│ [Linear API Token 설정 영역]        │
│                                     │
│ ─────────────────────────────────── │
│ Version 1.1.1    [업데이트 확인]    │
└─────────────────────────────────────┘
```

### 개발 환경에서 테스트

1. `dev-app-update.yml` 확인 (프로젝트 루트)
2. `auto-updater.ts`에서 `forceDevUpdateConfig = true` 주석 해제
3. GitHub에 테스트용 높은 버전 릴리즈 생성
4. `npm start`로 실행 → 업데이트 감지 확인

### 트러블슈팅

| 증상 | 원인 | 해결 |
|------|------|------|
| 업데이트 확인 안 됨 | 개발 모드에서는 비활성화 | `app.isPackaged`가 true인 환경에서 테스트 |
| 다운로드 실패 | GitHub API rate limit | 잠시 후 재시도 |
| 설치 후 권한 오류 | TCC 캐시 문제 | `tccutil reset ScreenCapture com.gpters.linear-capture` |
